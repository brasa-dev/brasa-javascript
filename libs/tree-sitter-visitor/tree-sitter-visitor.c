...
void noop (TSNode node, struct visit_context * context) {}
...
// set both type 'enter' and 'exit' global visit functions
bool context_set_global_visitor(struct visit_context * context, void (*enter)(), void (*exit)()) {
...
void visit_tree_cursor (TSNode tree, struct visit_context * context) {
  ...
  while (true) {
    if (recurse && ts_tree_cursor_goto_first_child(context->cursor)) {
      ...
      enter = context->globalVisitor->enter;
      enter(node, context);
      if (visitor != NULL) {
        enter = visitor->enter;
        if (enter != NULL) {
      ...
      exit = context->globalVisitor->exit;
      exit(node, context);
...
void _debug_tree (TSNode node, struct visit_context * context) {
  ...
  printf("Id: %lu  %s  %s %d  %s  %hu\t%s\t", (uintptr_t) node.id, in, out, isNamed, type, sym, text);
...
void visit_tree (TSNode node, struct visit_context * context) {
  if (context->debug) {
    if (context->globalVisitor == NULL) {
      printf("No global Visitor\n");
    } else {
      printf("Global Visitor registered\n");
    }
    printf("Registered visitor types:\n");
    printf("------\n");
    if (context->globalVisitor == NULL) {
      struct visitor globalVis = {.enter=noop, .exit=noop};
      context->globalVisitor = &globalVis;
    }
